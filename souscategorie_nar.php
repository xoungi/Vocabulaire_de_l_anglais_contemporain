...
<html lang="fr">
	...
	<body>
		...
		<?php 
			$id_souscategorie = $_GET["id"];
			$result3 = pg_query($dbconn, "SELECT * FROM souscategorie WHERE id_souscategorie = ".$id_souscategorie." ORDER BY id_souscategorie");
			while ($row1 = pg_fetch_row($result3)) {
				echo "<hr />";
				echo "<h3><a href =\"souscategorie.php?id=$row1[0]\"> $row1[0] - $row1[1]</a> ;</h3>";
				/**
				 * Concerne les mots directement liés à la sous-catégorie
				 */
				$result3b = pg_query($dbconn, "SELECT id_groupedemot2 FROM vocabulaire WHERE id_souscategorie = ".$id_souscategorie." AND id_souscategorie = ".$row1[0]." AND id_branche=0 ORDER BY id");						
						...
						if (!empty($id_groupedemot)) {
							...
							foreach ($gp as $k2 => $v2) {
								...
								foreach ($g as $k3 => $v3) {
									...
									$result3e = pg_query($dbconn, "SELECT vocabulaire, vocabulaire_plus FROM vocabulaire WHERE id_souscategorie = ".$row1[0]." AND id = ".$v3." ORDER BY id");
									while ($rowe = pg_fetch_assoc($result3e)) {
										$vocabulaire = $rowe["vocabulaire"];
										$vocabulaire_plus = $rowe["vocabulaire_plus"];
									}
									echo "<b>".$vocabulaire." ; </b>";
									if ($vocabulaire_plus != "NULL") {
										echo " --- <b>".$vocabulaire_plus."</b> ;";
									}
								}
							}
						}
				...
				while ($row2 = pg_fetch_row($result4)) {
					echo "<hr />";
					echo "<h4> $row2[0] - $row2[1] ;</h4>";
					/**
					 * Concerne les mots directement liés à la branche
					 */
					$result4b = pg_query($dbconn, "SELECT id_groupedemot2 FROM vocabulaire WHERE id_branche = ".$row2[0]." AND id_souscategorie = ".$id_souscategorie." AND id_branche = ".$row2[0]." AND id_sousbranche=0 ORDER BY id");						
							...
							if (!empty($id_groupedemotb)) {
								...
								foreach ($gp2 as $k2b => $v2b) {
									...
									foreach ($g2 as $k3b => $v3b) {
										echo "<br />"; // Permet de sauter une ligne entre les mots
										$result4e = pg_query($dbconn, "SELECT vocabulaire, vocabulaire_plus FROM vocabulaire WHERE id_branche = ".$row2[0]." AND id = ".$v3b." ORDER BY id");
										while ($row2e = pg_fetch_assoc($result4e)) {
											$vocabulaireb = $row2e["vocabulaire"];
											$vocabulaireb_plus = $row2e["vocabulaire_plus"];
										}
										echo "<b>".$vocabulaireb." ; </b>";
										if ($vocabulaireb_plus != "NULL") {
											echo " --- <b>".$vocabulaireb_plus."</b> ;";
										}
									}
								}
							}
								...
								if (!empty($id_groupedemotc)) {
									...
									foreach ($gp3 as $k2c => $v2c) {
										...
										foreach ($g3 as $k3c => $v3c) {
											echo "<br />"; // Permet de sauter une ligne entre les mots
											$result5e = pg_query($dbconn, "SELECT vocabulaire, vocabulaire_plus FROM vocabulaire WHERE id_sousbranche = ".$row3[0]." AND id = ".$v3c." ORDER BY id");
											while ($row3e = pg_fetch_assoc($result5e)) {
												$vocabulairec = $row3e["vocabulaire"];
												$vocabulairec_plus = $row3e["vocabulaire_plus"];
											}
											echo "<b>".$vocabulairec." ; </b>";
											if ($vocabulairec_plus != "NULL") {
												echo " --- <b>".$vocabulairec_plus." </b> ;";
											}
										}
									}
								}
