			...
			while ($row = pg_fetch_row($result1)) {
				...
				while ($row = pg_fetch_row($result2)) {
					...
					while ($row = pg_fetch_row($result3)) {
						 ...
						 */
						echo "<ul>";
						$result3b = pg_query($dbconn, "SELECT vocabulaire, traduction, vocabulaire_plus, traduction_plus FROM vocabulaire WHERE id_souscategorie = ".$row[0]." AND id_branche=0 ORDER BY id");
						while ($rowb = pg_fetch_row($result3b)) {
							echo "<li> $rowb[0] ($rowb[1]) 
								";
								if ($rowb[2] != "NULL"){
									echo "--- ".$rowb[2]." : ".$rowb[3];
								}
						...
						while ($row = pg_fetch_row($result4)) {
							 ...
							 */
							echo "<ul>";
							$result4b = pg_query($dbconn, "SELECT vocabulaire, traduction, vocabulaire_plus, traduction_plus FROM vocabulaire WHERE id_branche = ".$row[0]." AND id_sousbranche=0 ORDER BY id");
							while ($rowc = pg_fetch_row($result4b)) {
								echo "<li> $rowc[0] ($rowc[1]) 
									";
									if ($rowc[2] != "NULL"){
										echo "--- ".$rowc[2]." : ".$rowc[3];
									}
							...
							while ($row = pg_fetch_row($result5)) {
								...
								* Concerne les mots directement liés à la sous-branche
								 */
								echo "<ul>";
								$result5b = pg_query($dbconn, "SELECT vocabulaire, traduction, vocabulaire_plus, traduction_plus FROM vocabulaire WHERE id_sousbranche = ".$row[0]." ORDER BY id");
								while ($rowd = pg_fetch_row($result5b)) {
									echo "<li> $rowd[0] ($rowd[1]) 
										";
										if ($rowd[2] != "NULL"){
											echo "--- ".$rowd[2]." : ".$rowd[3];
										}
